...
function maximumScore(treasures) { return 250 + treasures * 25; }
...
function selectShot(row,col){if(state.gameOver||state.config.mode==='cpu'&&state.current===1)return;const target=state.players[1-state.current].board[row][col];if(target?.hit||target?.miss){$('#gameMessage').textContent='Essa coordenada já foi atacada. Escolha outra.';return;}pendingShot={row,col};resolveShot();}
...
function finishChallenge(success,message){clearInterval(timerId);timerId=null;const attacker=state.players[state.current];if(success){attacker.stats.correct++;attacker.score+=5;state.history.unshift(`${attacker.name} ganhou +5 pela tabuada.`);$('#answerFeedback').textContent=`${message} +5 pontos!`;}else{attacker.stats.wrong++;state.history.unshift(`${attacker.name} não ganhou o bônus da tabuada.`);$('#answerFeedback').textContent=`${message} O acerto continua valendo.`;}renderGame();setTimeout(()=>{$('#challengeDialog').close();if(!checkVictory())endTurn(true);},650);}

function resolveShot(){const attacker=state.players[state.current],defender=state.players[1-state.current],{row,col}=pendingShot,item=defender.board[row][col],coord=`${String.fromCharCode(65+row)}${col+1}`;let message,hit=false;if(!item){defender.board[row][col]={kind:'water',miss:true};attacker.stats.misses++;message=`🌊 Água em ${coord}.`;state.history.unshift(`${attacker.name}: água em ${coord}.`);}else if(item.kind==='treasure'){item.hit=true;attacker.score+=20;attacker.stats.hits++;state.bonusShots++;hit=true;message=`💰 Tesouro em ${coord}! +20 pontos e um tiro extra. Agora tente +5!`;state.history.unshift(`${attacker.name} encontrou um tesouro!`);}else{item.hit=true;attacker.score+=5;attacker.stats.hits++;hit=true;const cells=defender.board.flat().filter(x=>x?.kind==='ship'&&x.id===item.id);const sunk=cells.every(x=>x.hit);if(sunk)cells.forEach(x=>x.sunk=true);message=sunk?`💥 ${item.name} afundado! +5 pontos. Responda para ganhar +5!`:`🎯 Acertou em ${coord}! +5 pontos. Responda para ganhar +5!`;state.history.unshift(`${attacker.name}: ${sunk?'afundou um navio':'acertou'} em ${coord}.`);}$('#gameMessage').textContent=message;renderGame();if(!hit){if(!checkVictory())setTimeout(()=>endTurn(false),850);return;}if(state.config.mode==='cpu'&&state.current===1){attacker.stats.correct++;attacker.score+=5;state.history.unshift(`${attacker.name} ganhou +5 pela tabuada.`);renderGame();if(!checkVictory())setTimeout(()=>endTurn(true),850);return;}setTimeout(()=>openChallenge(row+1,col+1),500);}
